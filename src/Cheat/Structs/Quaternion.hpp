...
#ifndef GRAPIK_QUATERNION_H
#define GRAPIK_QUATERNION_H
...
Vector3 Quaternion::ToEuler(Quaternion q1) {
    float Rad2Degg = 360.0f / (M_PI * 2.0f);
    ...
    if (test>0.4995f*unit) {
        ...
        v.x = M_PI / 2.0f;
        v.z = 0;
        return Vector3::NormalizeAngles(v * Rad2Degg);
    }
    if (test<-0.4995f*unit) {
        v.y = -2.0f * atan2f (q1.y, q1.x);
        v.x = -M_PI / 2.0f;
