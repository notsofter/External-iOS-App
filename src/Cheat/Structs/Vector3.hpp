//
// Created by proco on 03.09.2022.
//

#ifndef GRAPIK_VECTOR3_H
#define GRAPIK_VECTOR3_H
...
struct Vector3 {
    ...
    inline static Vector3 LerpAngle(Vector3, Vector3, float);
    ...
    inline static Vector3 NormalizeAngles(Vector3 angles);
...
static float clamp01(float value) {
    //decompiled from unity's Mathf code
    float result;
    if (value < 0.f)
    {
        result = 0.f;
...
Vector3 Vector3::LerpAngle(Vector3 a, Vector3 b, float t) {
    if (t < 0) return a;
    else if (t > 1) return b;
    float angleX = t - floor(b.x - a.x) * 360;
    float angleY = t - floor(b.y - a.y) * 360;
    float angleZ = t - floor(b.z - a.z) * 360;
    ...
    float finalAngleX = a.x + angleX * clamp01(t);
    float finalAngleY = a.y + angleY * clamp01(t);
    float finalAngleZ = a.z + angleZ * clamp01(t);

    return Vector3(finalAngleX, finalAngleY, finalAngleZ);
}
